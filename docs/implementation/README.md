# Implementation Guides

[← Documentation Home](../README.md)

Unity development guides for building the co-located multi-user VR system.

## Overview

Follow these guides for implementing VR features using Unity 2022.3 LTS, XR Interaction Toolkit, Meta XR SDK, and Unity Netcode for GameObjects.

- [VR Setup Guide](vr-setup.md)
- [Avatar Synchronization](avatar-synchronization.md)
- [Tracking Systems](tracking-systems.md)
- [Collision Detection](collision-detection.md)
- [Safety Protocols](safety-protocols.md)
- [Guardian Integration](guardian-integration.md)
- [Synchronization Patterns](synchronization-patterns.md)
- [Network Optimization](network-optimization.md)
- [Data Collection](data-collection.md)
- [Metrics Tracking](metrics-tracking.md)
- [Component Relationships](component-relationships.md)
- [VR Performance](vr-performance.md)
- [Network Performance](network-performance.md)
- [Common Issues](common-issues.md)
- [Showcase Scenarios](showcase-scenarios.md)

---

Related sections: [Architecture](../architecture/README.md) · [Workflows](../workflows/README.md)
